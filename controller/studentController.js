...
module.exports = {
    ...
    getAllplayQuiz: async (req, res, next) => {
        try {
    ...
    postResult: async(req,res,next) =>{
        try {
            const { courseID, pmarks,subject,status,userId,marks } = req.body;

            const newQuiz = await new Result({courseID, pmarks,subject,status,userId,marks })
            await newQuiz.save()
    ...
    getResult: async (req, res, next) => {
        try {
            ...
            if (result.length === 0) {
                return res.status(404).json({ message: "You havent given any Quiz." })
            }
