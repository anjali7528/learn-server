...
const validateCreateQuiz = require('../validation/validateCreateQuiz')
...
module.exports = {
    ...
    createQuiz: async (req, res, next) => { 
        try {
            ...
            const { courseid, pmarks,subject,questions,creatorID } = req.body;

            const newQuiz = await new Quiz({courseid,subject,pmarks,questions,creatorID})
            await newQuiz.save()
    ...
    getAllQuiz: async (req, res, next) => {
        try {
            ...
            const allQuiz = await Quiz.find({creatorID})
            if (allQuiz.length === 0) {
                return res.status(404).json({ message: "You havent made any Quiz." })
            }
            
            res.status(200).json({allQuiz})
        }
        catch (err) {
            res.status(400).json({ message: `error in getting all Quiz", ${err.message}` })
        }
    },
